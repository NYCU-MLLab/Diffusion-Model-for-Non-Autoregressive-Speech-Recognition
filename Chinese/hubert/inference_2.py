...
def test(tokenizer, df, diffuser, feature_extractor, model, device, args):
    ...
    return jiwer.cer(texts_true, texts_pred), total, T
...
def main():
    ...
    epoch, cer = min(enumerate(log), key=lambda x: x[1])
    ...
    feature_extractor = HubertModel.from_pretrained('facebook/hubert-base-ls960').feature_extractor.to(device)
    ...
    cer, total, T = test(tokenizer, df, diffuser, feature_extractor, model, device, args)
