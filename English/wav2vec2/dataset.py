...
class LibriSpeech(Dataset):
    ...
    def __init__(self, path, tokenizer, train):
        ...
        if train:
            self.logits = [torch.zeros(feature.size(0), len(tokenizer)) for feature in self.features]
            self.alignments = [torch.zeros(300, feature.size(0), dtype=torch.int8) for feature in self.features]
